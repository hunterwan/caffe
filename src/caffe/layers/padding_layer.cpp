  ...
  if (pad_pos_) {
	...
	for (int h = 0; h < height_in_; ++h) {
	  ...
	  caffe_copy(width_in_,
	     bottom[0]->cpu_data() + bottom[0]->offset(n, c, h),
	     top[0]->mutable_cpu_data() + top[0]->offset(n, c, h + pad_beg_, pad_beg_));
	}
      }
    }
  }
	...
	for (int h = 0; h < height_out_; ++h) {
	  ...
	  caffe_copy(width_out_,
	     bottom[0]->cpu_data() + bottom[0]->offset(n, c, h - pad_beg_, - pad_beg_),
	     top[0]->mutable_cpu_data() + top[0]->offset(n, c, h));
	}
      }
    }
  }
  ...
  if (pad_pos_) {
	...
	for (int h = 0; h < height_in_; ++h) {
	  ...
	  caffe_axpy(width_in_, (Dtype)1.,
	     top[0]->cpu_diff() + top[0]->offset(n, c, h + pad_beg_, pad_beg_),
	     bottom[0]->mutable_cpu_diff() + bottom[0]->offset(n, c, h));
	}
      }
    }
  }
	...
	for (int h = 0; h < height_out_; ++h) {
	  ...
	  caffe_axpy(width_out_, (Dtype)1.,
	     top[0]->cpu_diff() + top[0]->offset(n, c, h),
	     bottom[0]->mutable_cpu_diff() + bottom[0]->offset(n, c, h - pad_beg_, - pad_beg_));
	}
      }
    }
  }
...
void PaddingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (pad_pos_) {
	...
	CUDA_CHECK(cudaMemcpy2D(
	    top[0]->mutable_gpu_data() + top[0]->offset(n, c, pad_beg_, pad_beg_), sizeof(Dtype) * width_out_,
	    bottom[0]->gpu_data() + bottom[0]->offset(n, c), sizeof(Dtype) * width_in_,
    ...
    for (int n = 0; n < num_; ++n) {
      for (int c = 0; c < channels_; ++c) {
	CUDA_CHECK(cudaMemcpy2D(
	    top[0]->mutable_gpu_data() + top[0]->offset(n, c), sizeof(Dtype) * width_out_,
	    bottom[0]->gpu_data() + bottom[0]->offset(n, c, - pad_beg_, - pad_beg_), sizeof(Dtype) * width_in_,
  ...
  if (pad_pos_) {
	...
	for (int h = 0; h < height_in_; ++h) {
	  ...
	  caffe_gpu_axpy(width_in_, (Dtype)1.,
	     top[0]->gpu_diff() + top[0]->offset(n, c, h + pad_beg_, pad_beg_),
	     bottom[0]->mutable_gpu_diff() + bottom[0]->offset(n, c, h));
	}
      }
    }
  }
	...
	for (int h = 0; h < height_out_; ++h) {
	  ...
	  caffe_gpu_axpy(width_out_, (Dtype)1.,
	     top[0]->gpu_diff() + top[0]->offset(n, c, h),
	     bottom[0]->mutable_gpu_diff() + bottom[0]->offset(n, c, h - pad_beg_, - pad_beg_));
	}
      }
    }
  }
